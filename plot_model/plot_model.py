...
try:
    from tensorflow.python.keras.engine.functional import (
        Functional as Functional_or_Network,
    )
...
def is_wrapped_model(layer):
    ...
    return isinstance(layer, wrappers.Wrapper) and isinstance(
        layer.layer, Functional_or_Network
    )
    ...
    for i, layer in enumerate(layers):
            ...
            if inputs.findall(class_name_lower):
                ...
                if config != 0:
                    ...
                    if dropout.findall(class_name_lower):
                        label = "{}:{}|{}".format(
                            layer_name, class_name, config["rate"]
                        )
        ...
        if not expand_nested or not isinstance(layer, Functional_or_Network):
            if color is True:
                ...
                if inputs.findall(class_name_lower):
                    node = pydot.Node(
                        layer_id, label=label, fillcolor="deeppink", style="filled"
                    )
                elif conv.findall(class_name_lower):
                    node = pydot.Node(
                        layer_id, label=label, fillcolor="cyan", style="filled"
                    )
                elif pool.findall(class_name_lower):
                    node = pydot.Node(
                        layer_id, label=label, fillcolor="chartreuse", style="filled"
                    )
                elif normalization.findall(class_name_lower):
                    node = pydot.Node(
                        layer_id, label=label, fillcolor="dodgerblue1", style="filled"
                    )
                elif activation.findall(class_name_lower):
                    node = pydot.Node(
                        layer_id, label=label, fillcolor="pink", style="filled"
                    )
                elif dropout.findall(class_name_lower):
                    node = pydot.Node(
                        layer_id, label=label, fillcolor="darkorange", style="filled"
                    )
                elif dense.findall(class_name_lower):
                    node = pydot.Node(
                        layer_id, label=label, fillcolor="darkorchid1", style="filled"
                    )
                elif padding.findall(class_name_lower):
                    node = pydot.Node(
                        layer_id, label=label, fillcolor="beige", style="filled"
                    )
                elif concatenate.findall(class_name_lower):
                    node = pydot.Node(
                        layer_id, label=label, fillcolor="tomato", style="filled"
                    )
                elif (
                    ...
                    node = pydot.Node(
                        layer_id, label=label, fillcolor="yellow1", style="filled"
                    )
                else:
                    node = pydot.Node(
                        layer_id, label=label, fillcolor="gold", style="filled"
                    )
            else:
